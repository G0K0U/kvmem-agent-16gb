...
**本次控制器已包含 Bonsai/NInfer 接入，但自定义引擎和转换制品仍是独立前置条件，不由此扩展安装。** 详见 [Bonsai 配置](../../docs/BONSAI.md)和[完整部署手册](../../docs/DEPLOYMENT.zh-CN.md)。安装脚本经过语法、配置合并和控制器测试，尚未在第二台干净 CUDA 机器上完整重装。
