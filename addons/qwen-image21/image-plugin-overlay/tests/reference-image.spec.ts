...
describe('reference image compatibility boundary', () => {
  it('resolves generated images from presentation metadata without adding vision blocks to model history', () => {
    const ref=imageRef('generated'), message=sourcedMessage([{type:'text',text:'Generated image'}],{kind:'tool',callId:'generated-call'})
    const agent={session:{deriveMessages:()=>[message],snapshotEvents:()=>[{type:'tool/result',data:{message,meta:{kind:'dsh-image-gen',attachment:ref}}}]}}
    const resolved=referenceMessages(agent)
    expect(findReferenceImages(resolved)).toEqual([ref])
    expect(findReferenceImages(resolved,['generated'])).toEqual([ref])
    expect(findReferenceImages([message])).toEqual([])
    expect(referenceMessages({session:{...agent.session,deriveMessages:()=>[]}})).toEqual([])
  })
