...
import { withLocalImageGpu, type RewriteResult } from './local-gpu.js'
import { QWEN_T2I, QWEN_I2I, isAdvancedQwen, applyQwenRewrite } from './qwen-advanced.js'
...
export const inject = ['tools', 'attachments', 'credentials', 'webServer', 'localImageGpu']
...
export function apply(ctx: Context, config: Config = {}): void {
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec): Promise<GeneratedValue> {
      ...
      if (active.provider === 'comfyui') {
        ...
        const advanced = isAdvancedQwen(workflow.name, active.baseURL)
        if (advanced && workflow.name !== QWEN_T2I) throw new Error('Use edit_image for the Qwen I2I workflow')
        let rewrite: RewriteResult | undefined
        const generated = await withLocalImageGpu(ctx, active.baseURL, exec, () => generateComfyUIImage({
          ...
          signal: exec.signal,
          ...(advanced ? { transformWorkflow: (graph, images) => applyQwenRewrite(graph, rewrite, images) } : {}),
        }), advanced ? async () => { rewrite = await ctx.localImageGpu.rewrite({ prompt: mergeComfyUIPrompt(workflow.presetPrompt, args.prompt), images: [] }, exec.signal) } : undefined)
        const value = await saveGenerated(ctx, generated, active.provider, workflow.name, 'API workflow', current(), exec, knownWorkspaceRoots)
        if (rewrite) Object.assign(value, { rewritePrompt: rewrite.prompt, rewriteLog: rewrite.logPath, rewriteModel: rewrite.model })
        return value
      }
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec): Promise<GeneratedValue> {
      ...
      if (active.provider === 'comfyui') {
        const defaultEdit = active.workflow && isAdvancedQwen(active.workflow.name, active.baseURL) ? QWEN_I2I : undefined
        const workflow = selectComfyUIWorkflow(active, args.workflow || defaultEdit)
        const advanced = isAdvancedQwen(workflow.name, active.baseURL)
        if (advanced && workflow.name !== QWEN_I2I) throw new Error('Use generate_image for the Qwen T2I workflow')
        if (sourceImages.length > 16) throw new Error('Qwen-Image editing supports at most 16 references')
        if (sourceImages.length > 1 && !advanced) {
        ...
        let rewrite: RewriteResult | undefined
        const generated = await withLocalImageGpu(ctx, active.baseURL, exec, () => editComfyUIImage({
          baseURL: active.baseURL,
          workflowJson: workflow.json,
          prompt: mergeComfyUIPrompt(workflow.presetPrompt, args.prompt),
          sourceImage: { data: sourceImage.data, mediaType: sourceImage.mediaType },
          ...(advanced ? { sourceImages, transformWorkflow: (graph, images) => applyQwenRewrite(graph, rewrite, images) } : {}),
          timeoutMs: active.timeoutMs,
          maxBytes: ctx.attachments.imageLimits.maxImageBytes,
          signal: exec.signal,
        }), advanced ? async () => { rewrite = await ctx.localImageGpu.rewrite({ prompt: mergeComfyUIPrompt(workflow.presetPrompt, args.prompt), images: sourceImages }, exec.signal) } : undefined)
        const value = await saveGenerated(ctx, generated, active.provider, workflow.name, 'API workflow', current(), exec, knownWorkspaceRoots)
        if (rewrite) Object.assign(value, { rewritePrompt: rewrite.prompt, rewriteLog: rewrite.logPath, rewriteModel: rewrite.model })
        return value
      }
  ...
  return {
    schema: {
      type: 'object', additionalProperties: false, properties: {
        ...
        rewritePrompt: { type: 'string' }, rewriteLog: { type: 'string' }, rewriteModel: { type: 'string' },
      },
    },
    render: (_args: unknown, value: GeneratedValue) => {
      ...
      return [
        ...
        // The local chat backends do not accept the generated WebP as vision input.
        // presentationMeta/presentResult still display the image in the conversation.
        ...(value.provider === 'comfyui' && (value.model === 'Qwen-Image 2.1 Q4_K_M' || [QWEN_T2I, QWEN_I2I].includes(value.model))
    ...
    presentationMeta: (args: unknown, value: GeneratedValue) => ({
      ...
      ...(value.rewritePrompt ? { rewritePrompt: value.rewritePrompt, rewriteLog: value.rewriteLog, rewriteModel: value.rewriteModel } : {}),
