...
export async function resolveReferenceImages(input: {
  ...
  const refs = findReferenceImages(referenceMessages(input.agent), sourceAttachmentIds)
  if (refs.length === 0) {
...
/** Local image results use text for the model, but retain their image in tool presentation metadata.
 * Restore those refs only for image selection, never into the model's request history.
 */
export function referenceMessages(agent: ReferenceImageAgent): readonly Message[] {
  const byCall = new Map<string, ImageAttachmentRef>()
  for (const event of agent.session.snapshotEvents?.() ?? []) {
    const item = record(event)
    if (item?.type !== 'tool/result') continue
    const data = record(item.data), meta = record(data?.meta), message = record(data?.message), source = record(message?.source)
    if (meta?.kind !== 'dsh-image-gen' || typeof source?.callId !== 'string') continue
    const ref = parseImageAttachmentRef(meta.attachment)
    if (ref) byCall.set(source.callId, ref)
  }
  return agent.session.deriveMessages().map(message => {
    if (message.source?.kind !== 'tool' || collectInBlocks(message.content).length) return message
    const ref = byCall.get(message.source.callId)
    return ref ? { ...message, content: [...message.content, { type: 'image' as const, attachment: ref }] } : message
  })
}
