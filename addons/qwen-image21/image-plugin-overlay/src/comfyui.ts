...
interface ComfyUIJobInput {
  ...
  timeoutMs: number
  signal: AbortSignal
  transformWorkflow?: (workflow: Record<string, unknown>, images: string[]) => Record<string, unknown>
}
...
export async function generateComfyUIImage(input: ComfyUIJobInput & {
  ...
  const prepared = prepareComfyUIWorkflow(input.workflowJson, input.prompt, seed)
  const workflow = input.transformWorkflow?.(prepared, []) ?? prepared
...
export async function editComfyUIImage(input: ComfyUIJobInput & {
  ...
  sourceImages?: readonly ComfyUISourceImage[]
  maxBytes: number
}): Promise<GeneratedComfyUIImage> {
  const seed = randomSeed()
  const output = await runJob(input, async (baseURL, signal) => {
    const sources = input.sourceImages ?? (input.sourceImage ? [input.sourceImage] : [])
    if (!sources.length || sources.length > 16) throw new Error('ComfyUI editing requires 1–16 reference images')
    if (sources.length > 1 && !input.transformWorkflow) throw new Error('This workflow supports only one source image')
    const names: string[] = []
    for (const source of sources) names.push(await uploadSourceImage(baseURL, source, signal))
    const prepared = prepareComfyUIWorkflow(input.workflowJson, input.prompt, seed, names[0])
    const workflow = input.transformWorkflow?.(prepared, names) ?? prepared
    const promptId = await submitWorkflow(baseURL, workflow, signal)
    const result = await waitForOutput(baseURL, promptId, signal)
    return await downloadOutput(baseURL, result, input.maxBytes, signal)
  })
