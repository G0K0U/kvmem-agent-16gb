...
`
    ...
    function Panel(props) {
      ...
      const saveConfig = (restart = false) => {
        if (!cfgDraft) return
        const cfg = {
        ...
        scope.set('config', cfg).then(() => {
          if(restart === true) scope.set('action','restart').catch(console.error)
          setCfgSaved(true)
      ...
      const slotPickRow = (slotKey) => {
        ...
        const files = (s.files || []).filter((f) => !/mmproj/i.test(f))
        const rows = []
        if (files.length) {
          rows.push(h('div', { className: 'dsh-llm-bubble-row dsh-llm-wrap' },
            ...
            files.map((f) => h('button', {
              key: f,
              className: 'dsh-llm-bubble dsh-llm-file-bubble' + (s.file === f ? ' on' : ''),
              title: f,
      ...
      const modelFolderSection = () => {
        if (!cfgDraft || !cfgDraft.slots || !cfgDraft.slots[st.slot]) return null
        const active = st.slot === 'b' ? 'b' : 'a'
        const s = cfgDraft.slots[active]
      ...
      const quickControls = () => {
        ...
        const field=(flag,label,values)=>{
          const index=rows.findIndex(r=>r.flag===flag)
        ...
        return h('div',{className:'dsh-llm-bubble-group'},h('strong',null,'KVMem 快捷参数'),
          ...
          field('--kvmem-gen-reserve','生成预留',[4096,8192,16384]),
          h('p',null,'当前推荐 64K / MTP2 / KV 32K + 预留 8K。更大上下文与缓存为实验配置。'),
          h('button',{className:'dsh-llm-btn primary',disabled:st.status==='starting'||st.status==='stopping',onClick:()=>saveConfig(true)},'应用并重启'))
      }
      ...
      return h('div', { className: 'dsh-llm-card' + (open ? ' dsh-open' : '') },
        ...
        open ? h('div', { className: 'dsh-llm-body' },
          ...
          argRows(),
          h('div', { className: 'dsh-llm-bubble-row' },
            h('button', { className: 'dsh-llm-btn', disabled: !cfgDraft, onClick: () => saveConfig(false) }, t('btn.save')),
            h('button', {className:'dsh-llm-btn primary',disabled:!cfgDraft || st.status==='starting' || st.status==='stopping',onClick:()=>saveConfig(true)}, '应用并重启'),
    ...
    // ---- plugin ----
    // DSH client runner (0.1.2+) resolves services only when declared in the
    // module's inject list — undeclared ctx access throws ("cannot get property
    // ... without inject") and silent lookups come back undefined. locale /
    // slots / settingsScope are all first-party shell services; declaring them
    // matches the official client-plugin pattern (see dsh-client-ui-settings-*).
